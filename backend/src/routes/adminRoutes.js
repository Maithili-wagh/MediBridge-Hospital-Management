...
router.get("/dashboard", protect, allowRoles("admin"), async (_req, res) => {
...
router.get("/users", protect, allowRoles("admin"), async (_req, res) => {
  res.json(await User.find().select("-password").sort({ role: 1, name: 1 }));
});

router.patch("/users/:userId/status", protect, allowRoles("admin"), async (req, res) => {
...
router.post("/departments", protect, allowRoles("admin"), async (req, res) => {
  const department = await Department.create(req.body);
  res.status(201).json(department);
});

router.patch("/departments/:id", protect, allowRoles("admin"), async (req, res) => {
  const department = await Department.findByIdAndUpdate(req.params.id, req.body, { new: true });
  if (!department) return res.status(404).json({ message: "Department not found." });
  res.json(department);
});

router.patch("/doctors/:userId/approval", protect, allowRoles("admin"), async (req, res) => {
...
router.patch("/pharmacists/:userId/approval", protect, allowRoles("admin"), async (req, res) => {
  const { status } = req.body;
  if (!["approved", "rejected"].includes(status)) {
    return res.status(400).json({ message: "Approval status must be approved or rejected." });
  }
...
router.patch("/lab-technicians/:userId/approval", protect, allowRoles("admin"), async (req, res) => {
  const { status } = req.body;
  if (!["approved", "rejected"].includes(status)) {
    return res.status(400).json({ message: "Approval status must be approved or rejected." });
  }
