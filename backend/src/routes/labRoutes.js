...
router.get("/tests", protect, allowRoles("patient", "doctor", "lab_technician", "admin"), async (_req, res) => {
  await ensureDefaultTests();
  res.json(await LabTestCategory.find().sort({ category: 1, testName: 1 }));
});

router.post("/tests", protect, allowRoles("admin"), async (req, res) => {
  const test = await LabTestCategory.create(req.body);
  res.status(201).json(test);
});

router.patch("/tests/:id", protect, allowRoles("admin"), async (req, res) => {
...
router.post("/requests", protect, allowRoles("doctor", "admin"), async (req, res) => {
...
router.get("/requests", protect, allowRoles("doctor", "lab_technician", "admin"), async (req, res) => {
...
router.get("/requests/mine", protect, allowRoles("patient"), async (req, res) => {
  res.json(await LabRequest.find({ patientEmail: req.user.email }).populate(["doctor", "test"]).sort({ createdAt: -1 }));
});

router.get("/requests/doctor", protect, allowRoles("doctor"), async (req, res) => {
  res.json(await LabRequest.find({ patientEmail: { $exists: true } }).populate(["doctor", "test", "appointment"]).sort({ createdAt: -1 }));
});

router.patch("/requests/:id/book", protect, allowRoles("patient", "admin"), async (req, res) => {
...
router.patch("/requests/:id/collect", protect, allowRoles("lab_technician", "admin"), async (req, res) => {
...
router.patch("/requests/:id/results", protect, allowRoles("lab_technician", "admin"), async (req, res) => {
  const request = await LabRequest.findByIdAndUpdate(
    req.params.id,
    {
...
router.patch("/requests/:id/review", protect, allowRoles("doctor", "admin"), async (req, res) => {
  const request = await LabRequest.findByIdAndUpdate(
    req.params.id,
...
router.get("/requests/:id/report", protect, allowRoles("patient", "doctor", "lab_technician", "admin"), async (req, res) => {
...
router.get("/requests/:id/report-file", protect, allowRoles("patient", "doctor", "lab_technician", "admin"), async (req, res) => {
  const filter = req.user.role === "patient"
    ? { _id: req.params.id, patientEmail: req.user.email }
    : { _id: req.params.id };
